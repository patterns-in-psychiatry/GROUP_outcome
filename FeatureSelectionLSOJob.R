...
siteLabel <- read.delim(siteCoding,header = F)
Fmat <- Fmat[siteLabel!=site,]
...
sumFunc <- function (data,lev = NULL,model = NULL){
  out = confusionMatrix(data[, "pred"], data[, "obs"])$byClass
  out
}
...
# FselSetup$iters <- 100
FselSetup$sizes <- 1:153
FselSetup$control$number <- Kmid
#FselSetup$control$p = .9
